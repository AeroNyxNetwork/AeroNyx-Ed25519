...
use crate::config::constants::{AUTH_CHALLENGE_TIMEOUT, CHALLENGE_SIZE};
...
mod tests {
    ...
    async fn test_challenge_lifecycle() {
        ...
        // Should fail because we didn't sign with the right key
        assert!(result.is_err());
        
        // Add another challenge and verify expiration cleanup
        let challenge2 = challenge_manager.generate_challenge(client_addr).await.unwrap();
        
        // Simulate expiration
        // Note: we can't easily manipulate the Instant in the tests,
        // so this is just a logical test of the cleanup function
        let removed = challenge_manager.cleanup_expired().await;
        assert_eq!(removed, 0); // None should be expired yet
        
        // Final count should still be 1 - we cleaned one manually during verify
        assert_eq!(challenge_manager.challenge_count().await, 1);
    }
